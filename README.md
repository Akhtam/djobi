...
- A Postgres database — local or in the cloud (see [Choosing a database](#choosing-a-database))

## 1. Get the server side running

Pick one option. Both end with the backend and dashboard running and the database migrated.
Everything from step 2 onward is the same either way.
...
The backend connects over the standard Postgres wire protocol (`pg`), not a cloud-specific driver
(see `docs/adr/0002-postgres-driver-for-local-dev.md`), so any of these work. Pick one:

- **A serverless cloud database** (e.g. [Neon](https://neon.tech), [Supabase](https://supabase.com),
  or another Postgres-compatible host): create a project there and copy its connection string
...
> [!TIP]
> **Want your job applications stored somewhere more durable? Use a cloud database.** A local
> Postgres or Docker volume lives only on this machine, so a wiped disk, a `docker compose down -v`
> or a new laptop takes your application history with it. A hosted Postgres like Neon or Supabase
> (both have free tiers) keeps your data safe off this machine, backed up by the provider, and
> reachable from any machine you run djobi on. Nothing else in the setup changes: just point
> `DATABASE_URL` at it.
...
Edit `apps/backend/.env`. `.env.example` documents all three `DATABASE_URL` shapes above. The local
Docker one is filled in by default, so replace it if you picked a cloud or local install:
...
- The bind address defaults to `127.0.0.1`, never `0.0.0.0`, on a bare `pnpm dev:backend` run
  ...
  backend service in Option B, where "every interface" means every other container on that one
...
This option always uses that bundled Postgres container (`docker-compose.yml` overrides
`DATABASE_URL`), so your applications live in a Docker volume on this machine. If you want them in a
cloud database instead, use Option A.
...
`vite dev`). Use Option A instead if you're changing backend or dashboard code.

`pnpm db:generate` regenerates migrations after changing `apps/backend/src/db/schema.ts`, the same
as Option A above. `docker compose down` stops everything, keeping the Postgres volume; add `-v` to
also drop it.
